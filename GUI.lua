...
function MPOWA:ApplyAttributesToButton(i, button)
	...
	button:SetID(i)
	_G("ConfigButton"..p.."_Icon"):SetTexture(MPOWA_SAVE[i]["texture"] or "Interface\\AddOns\\zzzModifiedPowerAuras\\images\\dummy.tga")
...
function MPOWA:Remove()
	if ConfigButton1 then
		...
		if not MPOWA_SAVE[self.selected+coeff] or not MPOWA_SAVE[self.selected+coeff]["used"] then return end
